...
public class GroupImporter extends Import<Group, GroupService>{
	...
	public GroupImporter() throws Exception {
		try
    	{
    		propertyDescriptors= BeanIntrospector.getProperties(Group.class);
    		beanProxy = new BeanProxy(new Group());    		
    		Annotation annotation = Group.class.getAnnotation(ConstraintImportTable.class);
    		if (annotation == null) 
    		{

    			LOGGER.error("No tiene anotaciones el modelo de Group");
    			throw new Exception("No tiene anotaciones el modelo de Group");
    		}
    		else
    		{
    		...
    		init(Group.class.getDeclaredFields());
    		
    	}catch (IntrospectionException e) {
    		LOGGER.error("Error al crear el beanProxy GroupImporter: "+e.getMessage());
		}
	}
		...
		while (reader.readRecord() && !tableError) {	
			...
			if(!rowError)
			{
				...
				else
				{
					...
					if(haveIdfilling(tableInfo.getKeys(),(Group) beanProxy.getBean())){
