...
public class ImageUtils {
	  ...
	  public void resizeAndSave(String imgPathOrign,int maxWidth, int maxHeight){
		  int imgType=getImageType(imgPathOrign);
		  if (imgType!=-1){ //si es un formato conocido
			  try {
				  BufferedImage image=resizeImage(imgPathOrign, imgType,maxWidth,maxHeight);
				  String imgPathDest=imgPathOrign.replace(".", "temp."); //El destino es un archivo temporal
				  saveImage(image, imgPathDest, imgType);
				  File source=new File(imgPathDest);
				  File destination=new File(imgPathOrign);
				  FileUtil.copy(source, destination); //Sobrescribimos la imagen original con la imagen con el nuevo tama�o
				  source.delete();//Borramos fichero temporal
			  }catch (Exception e) {
	  ...
	  private int getImageType(String imgPath){
		  int imageType=-1;
		  
		  if (imgPath.indexOf(".")>0){
			  imgPath=imgPath.substring(imgPath.indexOf(".")+1);
			  if (imgPath.equalsIgnoreCase("jpg")||imgPath.equalsIgnoreCase("jpeg"))
				  imageType=IMAGE_JPEG;
			  else if (imgPath.equalsIgnoreCase("png"))
				  imageType=IMAGE_PNG;
			  else if (imgPath.equalsIgnoreCase("gif"))
				  imageType=IMAGE_GIF;
		  }
		  return imageType;
	  }
	  ...
	  public static boolean saveImage(BufferedImage image, OutputStream outputStream, int type)
	  {
	    try {
